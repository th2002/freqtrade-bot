...
class Telegram(RPCHandler):
    ...
    def _load_whitelist(self):
        """
        Load whitelist from custom JSON file
        """
        whitelist_file = "user_data/custom_whitelist.json"
        try:
            with open(whitelist_file, 'r') as f:
                whitelist = json.load(f)
            self._config['exchange']['pair_whitelist'] = whitelist['whitelist']
            logger.info(f"Loaded {len(whitelist['whitelist'])} pairs from custom whitelist.")
    ...
    def _init(self) -> None:
        ...
        handles = [
            ...
            CommandHandler("whitelist_add", self._add_whitelist),
    ...
    @authorized_only
    async def _add_whitelist(self, update: Update, context: CallbackContext) -> None:
        """
        Handler for /add_whitelist.
        Adds a pair to the whitelist.
        """
        if context.args:
            ...
            try:
                ...
                if pair not in whitelist['whitelist']:
                    ...
                    self._config['exchange']['pair_whitelist'] = whitelist['whitelist']
                    await self._send_msg(f"Added {pair} to whitelist. Reloading config...")
                    await self._reload_config(update, context)
                else:
                    await self._send_msg(f"{pair} is already in the whitelist.")
            except Exception as e:
                await self._send_msg(f"Error adding pair to whitelist: {str(e)}")
        else:
            await self._send_msg("Please provide a pair to add to the whitelist.")

    def add_handlers(self, *args, **kwargs):
        # ... (existing code)
        self._dp.add_handler(CommandHandler('add_whitelist', self._add_whitelist))
    ...
    async def _reload_config(self, update: Update, context: CallbackContext) -> None:
        ...
        try:
            with open(whitelist_file, 'r') as f:
                whitelist = json.load(f)
            self._config['exchange']['pair_whitelist'] = whitelist['whitelist']
            await self._send_msg("Whitelist reloaded.")
        except Exception as e:
            await self._send_msg(f"Error reloading whitelist: {str(e)}")
